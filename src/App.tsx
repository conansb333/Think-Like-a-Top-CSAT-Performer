...
import { AVATAR_OPTIONS } from './components/WelcomeScreen';
...
export default function App() {
  ...
  // Host Play-vs-Spectate choice (set in the lobby before starting)
  const [hostPlays, setHostPlays] = useState<boolean>(false);
  const [hostNickname, setHostNickname] = useState<string>('Host');
  const [hostAvatarIdx, setHostAvatarIdx] = useState<number>(3); // FCR Lion default
  ...
  useEffect(() => {
    ...
    const pollRoom = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/multiplayer/room/${gameId}?clientId=${clientId}`);
        if (!res.ok) {
          if (res.status === 404) {
        ...
        if (responseData.success && responseData.game) {
          ...
          if (multiplayerMode === 'multiplayer_host') {
            ...
            if (game.status === 'playing') {
              // Host who chose to play enters the game like a regular player.
              // Host who chose to spectate sees the dashboard.
              if (hostPlays) {
                // Fall through to the player-sync block below (do not force host_dashboard).
              } else {
          ...
          // Runs for regular players, AND for a host who chose to play.
          const shouldSyncAsPlayer =
            multiplayerMode === 'multiplayer_player' ||
            (multiplayerMode === 'multiplayer_host' && hostPlays);
          if (shouldSyncAsPlayer) {
            if (game.status === 'lobby') {
              setPhase('lobby');
            } else if (game.status === 'playing') {
              if (phase === 'welcome' || phase === 'lobby' || phase === 'host_dashboard') {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (isSimulatedMultiplayer) {
        ...
        fetch(`${API_BASE}/api/multiplayer/answer`, {
          method: 'POST',
          ...
          if (data.success && data.game && isCorrect) {
            ...
            if (rewardRoll < 0.15) {
              ...
              if (targets.length > 0) {
                const target = targets[Math.floor(Math.random() * targets.length)];
                const stealPct = Math.random() < 0.5 ? 0.25 : 0.5;
                fetch(`${API_BASE}/api/multiplayer/chest`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
                    gameId,
  ...
  const handleJoinMultiplayer = async (
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/multiplayer/join`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          gameId: roomCode,
  ...
  const handleHostMultiplayer = async (
    ...
    try {
      setIsSimulatedMultiplayer(false);
      const res = await fetch(`${API_BASE}/api/multiplayer/create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
        ...
        <div className="w-full">
          ...
          {phase === 'lobby' && (
            <MultiplayerLobby
              ...
              hostPlays={hostPlays}
              onToggleHostPlays={() => setHostPlays(prev => !prev)}
              hostNickname={hostNickname}
              onSetHostNickname={setHostNickname}
              hostAvatarIdx={hostAvatarIdx}
              onSetHostAvatarIdx={setHostAvatarIdx}
              onStartGame={async () => {
                if (isSimulatedMultiplayer) {
                  setMultiplayerGame(prev => {
                    if (!prev) return null;
                ...
                // If the host chose to play, join as a regular player first,
                // then start the game so the host appears in the scoreboard.
                if (hostPlays) {
                  const av = AVATAR_OPTIONS[hostAvatarIdx] || AVATAR_OPTIONS[0];
                  try {
                    await fetch(`${API_BASE}/api/multiplayer/join`, {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({
                        gameId,
                        clientId,
                        name: hostNickname.trim() || 'Host',
                        avatar: av.name,
                        avatarEmoji: av.emoji,
                      }),
                    });
                  } catch (err) {
                    console.error('Host join-as-player failed:', err);
                  }
                }
                await fetch(`${API_BASE}/api/multiplayer/start`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
          ...
          {phase === 'host_dashboard' && multiplayerGame && (
            <HostDashboard
              ...
              onEnd={async () => {
                await fetch(`${API_BASE}/api/multiplayer/end`, {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ gameId, hostClientId: clientId }),
