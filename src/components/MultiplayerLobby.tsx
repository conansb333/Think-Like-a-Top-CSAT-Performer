...
import { Users, LogOut, Copy, Check, Sparkles, HelpCircle, Eye, Gamepad2 } from 'lucide-react';
import { MultiplayerPlayer } from '../types';
import { AVATAR_OPTIONS } from './WelcomeScreen';
...
interface MultiplayerLobbyProps {
  ...
  onAddBot?: () => void;
  hostPlays?: boolean;
  onToggleHostPlays?: () => void;
  hostNickname?: string;
  onSetHostNickname?: (v: string) => void;
  hostAvatarIdx?: number;
  onSetHostAvatarIdx?: (v: number) => void;
}
      ...
      <div className="bg-slate-900 border-2 border-slate-700/80 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 bg-slate-950/60 border border-slate-800 rounded-2xl p-5 flex flex-col justify-between space-y-6">
            ...
            {/* Host Play-vs-Spectate toggle */}
            {isHost && onToggleHostPlays && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => { if (hostPlays) onToggleHostPlays(); }}
                    className={`flex flex-col items-center justify-center gap-1 py-3 px-2 rounded-xl border-2 transition-all cursor-pointer ${
                      !hostPlays
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold uppercase">Spectate</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => { if (!hostPlays) onToggleHostPlays(); }}
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold uppercase">Play Too</span>
                  </button>
                </div>

                {/* When playing, collect host identity */}
                {hostPlays && (
                  <div className="space-y-2.5 pt-1">
                    <input
                      type="text"
                      value={hostNickname}
                      onChange={(e) => onSetHostNickname?.(e.target.value)}
                      placeholder="Your call sign"
                      maxLength={15}
                      className="w-full bg-slate-950 border border-slate-800 text-slate-100 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:border-amber-500/50 placeholder:text-slate-600"
                    />
                    <div className="grid grid-cols-6 gap-1.5">
                      {AVATAR_OPTIONS.map((av, idx) => (
                        <button
                          key={av.name}
                          type="button"
                          onClick={() => onSetHostAvatarIdx?.(idx)}
                          title={av.name}
                          className={`aspect-square rounded-lg flex items-center justify-center text-lg transition-all cursor-pointer border ${
                            hostAvatarIdx === idx
